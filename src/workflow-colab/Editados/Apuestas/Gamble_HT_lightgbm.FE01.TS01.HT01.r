...
PARAM  <- list()
PARAM$experimento  <- "G.FE01.TS01.HT01.6510"

PARAM$exp_input  <- "G.FE01.TS01.TS6410"

PARAM$lgb_crossvalidation_folds  <- 5  #En caso que se haga cross validation, se usa esta cantidad de folds

PARAM$lgb_semilla  <- 108881   #cambiar por su propia semilla
...
PARAM$bo_lgb <- makeParamSet(
         ...
         makeIntegerParam("min_data_in_leaf", lower=  80L,   upper= 50000L),
         makeNumericParam("min_gain_to_split", lower = 0.0, upper = 100.0), #la incluyo en la Bayesian Optimization
         makeNumericParam("lambda_l1", lower = 0.0001, upper = 100.0),      #la incluyo en la Bayesian Optimization
         makeNumericParam("lambda_l2", lower = 0.0001, upper = 100.0)       #la incluyo en la Bayesian Optimization
        )
